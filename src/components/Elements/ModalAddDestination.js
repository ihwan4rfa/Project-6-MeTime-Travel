...
const ModalAddDestination = ({ showAddDestination, setShowAddDestination }) => {
    ...
    return (
        <>
            <div className={`absolute z-20 w-full h-full opacity-40 bg-primaryblack ${showAddDestination === true ? '' : 'hidden'}`}></div>
            <div className={`${showAddDestination === true ? '' : 'hidden'} absolute z-30 flex items-center justify-center w-full h-full`}>
                <div className={`bg-white shadow-lg rounded-lg text-[13px] flex justify-center relative text-primaryblack h-fit w-[900px]`}>
                    ...
                    <form ref={formRef} onSubmit={handleUpdateDestination} className={`flex flex-col items-center justify-center w-full h-full p-5`}>
                        ...
                        <div className='z-10 flex items-center justify-between w-1/2 pb-4'>
                            ...
                            <h1 className={`flex items-center justify-center text-white ${showNextStep === true ? 'bg-primaryblue' : 'bg-slate-200'} text-[11px] w-4 h-4 rounded-full font-semibold`}>2</h1>
                        </div>
                            ...
                            <div className='flex w-full gap-4'>
                                <div className='flex flex-col w-2/5 overflow-scroll no-scrollbar rounded-lg h-[207px] gap-3'>
                                    {addImageUrls !== null && addImageUrls.map((imageUrl, index) => (
                                        <div key={index} className={`flex relative w-full ${addImageUrls.length === 1 ? 'h-full' : 'h-[75%]'}`}>
                                            <button onClick={() => handleRemoveImage(index)} type='button' className={`${addImageUrls[0] === '' ? 'hidden' : ''} absolute flex items-center justify-center m-2 bg-white rounded-full hover:bg-primaryred w-7 h-7 text-primaryred hover:text-white`}><i class="fa-regular fa-trash-can"></i></button>
                                ...
                                <div className='flex flex-col w-3/5 gap-4'>
                                    <div className='flex gap-4'>
                                        <input type="text" name="title" id="title" placeholder="Destination Name" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-4/6 outline-none" />
                                        <DropDownCategory
                                    ...
                                    <div className='flex gap-4'>
                                        <textarea name="description" id="description" placeholder="Description" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full h-24 outline-none" />
                                        <textarea name="facilities" id="facilities" placeholder="Facilities" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full outline-none" />
                                    </div>
                                    <div className='flex gap-4'>
                                        <input type="number" name="price" id="price" placeholder="Price" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full outline-none" />
                                        <input type="number" name="price_discount" id="price_discount" placeholder="Discount Price" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full outline-none" />
                                    </div>
                                </div>
                            </div>
                            <div className='flex w-full gap-4'>
                                <div class="bg-slate-200 text-slate-400 px-4 text-[13px] text-start rounded-lg w-2/5 flex items-center overflow-hidden whitespace-nowrap">
                                    <label htmlFor="addImageUrls" className="bg-slate-300 text-primaryblack w-fit cursor-pointer py-[10px] -ml-4 px-4 rounded-l-lg">Add Image</label>
                                    <span className={`px-4 overflow-hidden text-ellipsis ${addImageUrls ? 'text-primaryblack' : ''}`}>{fileName === null ? 'No File Selected' : `${fileName}`}</span>
                                </div>
                                <input onChange={handleUpload} type="file" name="addImageUrls" id="addImageUrls" className="hidden" />
                                <div className='flex w-3/5 gap-4'>
                                    <input type="number" name="rating" id="rating" placeholder="Rating" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full outline-none" />
                                    <input type="number" name="total_reviews" id="total_reviews" placeholder="Total Reviews" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-full outline-none" />
                                </div>
                            </div>
                        </div>
                        <div className={`flex flex-col items-start justify-center w-full gap-4 h-fit ${showNextStep === true ? '' : 'hidden'}`}>
                            <div className='flex flex-col w-full gap-4'>
                                <div className='flex gap-4'>
                                    <input type="text" name="address" id="address" placeholder="Address" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-3/5 outline-none" />
                                    <input type="text" name="province" id="province" placeholder="Province" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-3/5 outline-none" />
                                </div>
                                <div className='flex gap-4'>
                                    <input type="text" name="city" id="city" placeholder="City" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-3/5 outline-none" />
                                    <input onChange={handleLinkMap} type="text" name="location_maps" id="location_maps" placeholder="Location Maps" className="bg-slate-200 placeholder:text-slate-400 text-primaryblack py-[10px] px-4 text-[13px] rounded-lg w-3/5 outline-none" />
                                </div>
                                {srcUrl &&
