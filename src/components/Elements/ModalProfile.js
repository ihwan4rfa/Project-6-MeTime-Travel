                ...
                <div className={`${showProfile === true ? '' : 'hidden'} absolute z-40 flex items-center justify-center w-full h-full`}>
                    <div className={`bg-white dark:bg-primaryblack shadow-lg rounded-lg flex justify-center relative text-primaryblack dark:text-slate-200 ${editProfile === true ? 'h-fit w-[85%] xs:w-[400px] lg:w-[500px] xl:w-[600px]' : 'h-fit w-fit'}`}>
                        <div className='absolute flex justify-end w-full p-2'>
                            <button onClick={editProfile === true ? handleEditProfile : handleShowProfile} className='w-8 h-8 text-base lg:text-lg xl:text-xl rounded-lg hover:text-primaryred'><i class=" fa-solid fa-xmark"></i></button>
                        </div>
                        <div className={`flex flex-col items-center justify-center w-full h-full ${editProfile === true ? "hidden" : ''}`}>
                            <h1 className='z-10 pt-5 pb-3 lg:pb-4 font-medium'>My Profile</h1>
                            <div className='flex flex-col xs:flex-row items-center justify-center w-full h-full px-10 xl:px-12 gap-3 xs:gap-0'>
                                <div className='w-20 xs:w-16 lg:w-20 xl:w-24 h-20 xs:h-16 lg:h-20 xl:h-24 overflow-hidden rounded-lg'>
                                    <img src={user.profilePictureUrl} className='object-cover w-full h-full'></img>
                                </div>
                                <div className='flex flex-col items-center xs:items-start gap-2 ml-0 xs:ml-4 lg:ml-5 xl:ml-6'>
                                    <h1 className='text-sm  lg:text-base xl:text-lg font-semibold capitalize'>
                                        <i class={`fa-solid mr-1 lg:mr-[6px] xl:mr-2 ${user?.role === "admin" ? 'fa-user-plus text-primaryblue' : 'fa-user text-primaryred'}`}></i>
                                        {user.name}
                                    </h1>
                                    <div className='flex flex-col items-center xs:items-start gap-1 text-primarygray dark:text-slate-400'>
                        ...
                        <form onSubmit={handleUpdateUser} className={`flex flex-col items-center gap-3 lg:gap-4 p-5 justify-center w-full h-full ${editProfile === true ? '' : 'hidden'}`}>
                            ...
                            <div className='flex flex-col xs:flex-row items-center xs:items-start justify-center w-full gap-3 lg:gap-4 h-fit'>
                                ...
                                <div className='flex flex-col flex-1 w-full xs:w-1/3 gap-3 lg:gap-4'>
                            ...
                            <div className='flex flex-col xs:flex-row items-start justify-center w-full gap-3 xs:gap-4 h-fit'>
