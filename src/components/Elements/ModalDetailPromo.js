            ...
            <div className={`${showDetailPromo === true ? '' : 'hidden'} absolute z-30 flex items-center justify-center w-full h-full`}>
                <div className={`bg-white dark:bg-primaryblack shadow-lg rounded-lg text-[10px] lg:text-[11px] xl:text-[13px] flex justify-center relative text-primaryblack dark:text-slate-200 h-fit w-fit`}>
                    <div className='absolute flex justify-end w-full p-2'>
                        <button onClick={handleCloseEditPromo} className='w-8 h-8 text-base lg:text-lg xl:text-xl rounded-lg hover:text-primaryred dark:hover:text-primaryred'><i class=" fa-solid fa-xmark"></i></button>
                    </div>
                    <div className={`flex flex-col items-center justify-center w-full h-full pt-4 sm:pt-5 p-5`}>
                        <div className='flex flex-col items-start justify-center w-full h-fit'>
                            <div className='flex flex-col items-center sm:items-start sm:flex-row w-full gap-4'>
                                <h1 className='text-lg lg:text-xl xl:text-2xl font-semibold capitalize flex sm:hidden -mb-2'>{selectedPromo.title}</h1>
                                ...
                                <div className='flex flex-col w-[230px] xs:w-[300px] lg:w-[380px] xl:w-[450px] gap-1 text-primaryblack dark:text-slate-200'>
                                    <h1 className='text-lg lg:text-xl xl:text-2xl font-semibold capitalize hidden sm:flex'>{selectedPromo.title}</h1>
                                    <div className='flex flex-col xs:flex-row justify-between w-full pt-2 pb-3 xs:pb-2 pl-3 pr-2 mb-2 rounded-lg bg-slate-100 dark:bg-slate-700 gap-2 xs:gap-0'>
                                        <div className='flex flex-col items-start justify-between'>
                                            <h1 className='text-lg lg:text-xl xl:text-2xl font-semibold text-primaryblue'>{formatNumber(selectedPromo.promo_discount_price)}</h1>
                                        ...
                                        <div className='flex flex-col items-start xs:items-end justify-between gap-1 xs:gap-0'>
