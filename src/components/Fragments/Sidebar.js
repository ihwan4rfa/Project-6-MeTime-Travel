    ...
    return (
        <div className='w-1/6 h-screen bg-white'>
            <div className='flex flex-col gap-3 mt-24 text-[13px] font-medium'>
                <button onClick={() => router.push("/")} className='flex items-center justify-between px-6 py-2 mx-4 bg-white rounded-lg cursor-pointer hover:bg-slate-200'>
                    <h1>Preview</h1>
                    <h1><i class='fa-solid fa-globe text-primaryred'></i></h1>
                </button>
                <div className='h-[2px] mx-4 bg-secondaryyellow rounded-full'></div>
                {keys.map((key, index) => (
                    <button key={index} onClick={() => router.push(linkList[key])} className='flex items-center justify-between px-6 py-2 mx-4 bg-white rounded-lg cursor-pointer hover:bg-slate-200'>
                        <h1>{key}</h1>
