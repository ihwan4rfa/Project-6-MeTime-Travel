...
const Navbar = () => {
    ...
    return (
        <div className='font-poppins'>
            <nav className={`${currentPath.startsWith('/dashboard') ? 'px-10' : 'px-36'} ${showProfile || showModal || currentPath.startsWith('/dashboard') ? 'bg-opacity-100' : 'bg-opacity-[85%] backdrop-blur-lg dark:bg-opacity-[85%] dark:backdrop-blur-lg'} z-50 fixed flex items-center justify-between w-full h-16 bg-white dark:bg-primaryblack shadow-navbar dark:shadow-slate-600 text-primaryblack`}>
                <button onClick={() => currentPath.startsWith("/dashboard") ? router.push("/dashboard/users") : router.push(linkList.Home)}><Image className='w-auto h-9' src={isDark ? "/images/Logo-dark.png" : "/images/Logo.png"} width={500} height={500} /></button>
                        ...
                        <div key={index}>
                            <button
                                ...
                                className={`cursor-pointer dark:text-white hover:text-primaryred dark:hover:text-primaryyellow ${currentPath === modifiedLinkList[key] && currentPath === "/login_register" ? 'text-white bg-primaryred dark:bg-primaryyellow'
                                    : (currentPath === modifiedLinkList[key] ? 'text-primaryred dark:text-yellowprimary' : '')} ${key === "Login" ? "px-4 py-2 border hover:text-white dark:hover:text-white hover:bg-primaryred dark:hover:bg-primaryyellow rounded-lg border-primaryred dark:border-primaryyellow" : ""}`}>
                                {key}
                            </button>
                        </div>
                    ))}
                        ...
                        <div className='flex relative text-[13px] w-full'>
                            <button onClick={handleDropDownToggle} className='flex items-center w-fit'>
                                ...
                                <div className='text-left'>
                                    ...
                                    <p className='text-[11px] leading-3 text-primarygray dark:text-slate-400 capitalize'>{user.role}</p>
                                </div>
                            </button>
                            <div className={`absolute right-0 w-36 z-10 mt-14 bg-white dark:bg-primaryblack dark:text-slate-200 shadow-dropdown dark:shadow-slate-600 text-primaryblack rounded-lg ${dropDownHidden ? 'hidden' : ''}`}>
                                <div className="px-2">
                                    ...
                                    <button
                                        onClick={currentPath.startsWith("/dashboard") ? () => router.push("/") : () => router.push("dashboard/users")} className={`${user.role === "admin" ? '' : 'hidden'} flex items-center w-full justify-between px-4 py-1 my-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700`}>
                                    ...
                                    <button
                                        onClick={handleTheme} className='flex items-center justify-between w-full px-4 py-1 my-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700'>
                                        <h1>{isDark ? 'Light' : 'Dark'}</h1>
                                        <h1 className={`text-[11px] ${user.role === 'admin' ? 'text-primaryblue' : 'text-primaryred'}`}><i class={`fa-solid ${isDark ? 'fa-sun' : 'fa-moon mr-[2px]'}`}></i></h1>
                                    </button>
                                    <button
                                        onClick={logout} className='flex items-center justify-between w-full px-4 py-1 my-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700'>
                                        <h1>Logout</h1>
