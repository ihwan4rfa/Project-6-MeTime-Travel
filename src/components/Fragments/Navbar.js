...
const Navbar = () => {
    ...
    return (
        <div className='font-poppins'>
            <nav className={`${currentPath.startsWith('/dashboard') ? 'px-10' : 'px-36'} ${showProfile || showModal || currentPath.startsWith('/dashboard') ? 'bg-opacity-100' : 'bg-opacity-[85%] backdrop-blur-lg'} z-50 fixed flex items-center justify-between w-full h-16 bg-white shadow-navbar text-primaryblack`}>
                        ...
                        <div className='flex relative text-[13px] w-full'>
                            <button onClick={handleDropDownToggle} className='flex items-center w-fit'>
                                ...
                                <div className='text-left'>
                                    ...
                                    <p className='text-[11px] capitalize leading-3 text-primarygray'>{user.role}</p>
                                </div>
                            </button>
